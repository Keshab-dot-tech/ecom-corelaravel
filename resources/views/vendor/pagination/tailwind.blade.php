...
@if ($paginator->hasPages())
    <div class="flex flex-col items-center space-y-2 mt-6">
        ...
        <nav role="navigation" aria-label="Pagination" class="flex items-center space-x-2">
            ...
            {{-- Prev Button --}}
            @if ($paginator->onFirstPage())
                <span class="px-3 py-1 text-gray-400 bg-gray-100 rounded cursor-default">Prev</span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" 
                   class="px-3 py-1 text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-100">
                   Prev
                </a>
            @endif
            ...
            @foreach ($elements as $element)
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <span class="px-3 py-1 text-black bg-blue-600 border border-blue-600 rounded">{{ $page }}</span>
                        @else
                            <a href="{{ $url }}" 
                               class="px-3 py-1 text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-100">
                               {{ $page }}
            ...
            @else
                <span class="px-3 py-1 text-gray-400 bg-gray-100 rounded cursor-default">Next</span>
            @endif
        ...
        {{-- Showing X–Y of Z --}}
            ...
            <div class="showing text-sm text-gray-600">
            Showing 
            <span class="font-medium">{{ $paginator->firstItem() }}</span>
            –
            <span class="font-medium">{{ $paginator->lastItem() }}</span>
        ...
        {{-- <x-category.category :paginator="$paginator"/> --}}
